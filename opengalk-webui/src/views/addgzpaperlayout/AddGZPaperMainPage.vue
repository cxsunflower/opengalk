        ...
        <div v-for="subject in uploadForm.subjectArray" style="margin: 5px 5px 5px 5px;padding-left: 5px">
          ...
          >
            <div v-for="item in subject.items" v-show="subject.type !== 1" class="radio">
              <el-radio :label="item.value">{{ item.value }}、 {{ item.text }}
              </el-radio>
            </div>
          </el-radio-group>
...
const uploadPaper = () => {
  if (addOrUpdate == 0) {
    request.post(requestUrl + '/addGZPaper', uploadForm.value).then(result => {
      showMessage(result);
    ...
    request.put(requestUrl + '/updateGZPaper/' + uuid, uploadForm.value).then(result => {
      showMessage(result);
      if (result.data.响应状态 === 1) {
        uploadVisible.value = false;
      }
